...
=== "Smoldot (WebWorker)"
    ...
    ```typescript
    --8<-- "code/reference/tools/papi/api-instantiation-smoldot-webworker.ts"
    ```

=== "Smoldot (Node.js)"

    ```typescript
    --8<-- "code/reference/tools/papi/api-instantiation-smoldot-nodejs.ts"
    ```

=== "Smoldot"

    ```typescript
    --8<-- "code/reference/tools/papi/api-instantiation-smoldot.ts"
    ```

=== "WSS"

    ```typescript
    --8<-- "code/reference/tools/papi/api-instantiation-wss.ts"
    ```
...
- The `tx` method allows you to directly send a transaction with the specified parameters by using the `typedApi.tx.Pallet.Call` pattern:
    ...
    ```typescript
    --8<-- "code/reference/tools/papi/send-tx-transfer-keep-alive.ts"
    ```
