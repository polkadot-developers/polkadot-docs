...
description: Hands-on guide to publishing and subscribing to off-chain data via the Statement Store from a Polkadot Product — set up the client, subscribe by topic, publish typed statements, and use channels for last-write-wins state.
...
Three things to keep in mind before diving in:

- **Statements are signed.** Every submission carries an authentication proof (Sr25519 by default). The user's signing key lives in the Polkadot App on their phone; Polkadot Desktop holds a derived session context after pairing and routes signing requests to the Mobile App. The SDK requests proofs through the Host API, so your Product never handles keys directly and never holds the user's private material.
- **Submissions are allowance-gated.** Each account has an on-chain **statement allowance** capping the number of live statements and their total size. The pallet rejects submissions that would exceed the cap; subscribers don't have to deal with spam at the application layer.
- **Reading is permissionless.** Anyone connected to a People Chain node can subscribe to the public gossip stream; no per-subscriber authorization. Topic filters happen at the node, so subscribers only see traffic they've asked for.

This guide walks through the four hands-on steps for using it from a Product:

1. Set up the Statement Store client.
2. Subscribe to statements published by other instances of your Product.
3. Publish a typed statement.
4. Use a channel for last-write-wins state.
...
`client.subscribe<T>(callback, options?)` registers a topic filter with the connected node. Under the hood, the SDK calls `statement_subscribeStatement` with a filter scoped to your Product's `appName` topic; the node returns the current statement pool matching that filter and streams any further submissions that match. The SDK decodes the JSON payload into your typed `T` and dedupes by channel and content hash before invoking your callback:
...
`publish` returns `Promise<boolean>`: `true` when the node accepted the statement into its pool, `false` when it was rejected by `pallet-statement-store`'s validity check (typically allowance, size, or proof failure). It throws `StatementDataTooLargeError` if the JSON-encoded payload exceeds the per-statement size limit (**512 bytes**), and `StatementConnectionError` if the client isn't connected.

Two limits worth designing around:

- **512 bytes per statement payload** (after JSON encoding). For larger content, store it on the Bulletin Chain and publish the CID as a small statement.
- **Default 30-second TTL.** The pallet enforces a maximum retention window; the SDK defaults to 30 seconds and you can shorten or lengthen per-statement via `ttlSeconds` up to that cap. After expiry the node evicts the statement from its pool.
...
`ChannelStore` is the right primitive for soft state where only the latest version matters: presence indicators, multiplayer cursors, "now playing" status. For append-only events — chat messages, action logs, social-feed posts — keep using `client.publish` directly so each event lives independently until its TTL.

## Ship It

Once your Product publishes and subscribes to statements correctly against TestNet, ship it:
...
-   <span class="badge guide">Guide</span> **Read Chain State**
    ...
    For reading on-chain state alongside Statement Store subscriptions, use the Host API's PAPI provider.
