---
title: Open Message Passing Channels (HRMP)
...
Opening a channel between two parachains is a two-phase process, with one chain first initiating a channel request and then the second chain accepting it. When neither chain is a system chain, they will use the `hrmpInitOpenChannel` and `hrmpAcceptOpenChannel` extrinsics.
...
The
[`establish_channel_with_system`](https://paritytech.github.io/polkadot-sdk/master/polkadot_runtime_parachains/hrmp/pallet/dispatchables/fn.establish_channel_with_system.html){target=_blank}
