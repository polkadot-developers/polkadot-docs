...
To get the most out of this guide, ensure you're familiar with [FRAME concepts](/develop/rollups/customize-rollup/overview/){target=\_blank}.
...
2.  Configure the dependencies required for FRAME pallet development in the `Cargo.toml` file as follows:
    ...
    ```toml
    --8<-- 'code/develop/rollups/customize-rollup/make-custom-pallets/Cargo.toml'
    ```
    ...
    For detailed information about workspace inheritance and how to properly integrate your pallet with the runtime, see the [Add an Existing Pallet to the Runtime](/develop/rollups/customize-rollup/add-existing-pallets/){target=\_blank} page.

3.  Initialize the pallet structure by replacing the contents of `src/lib.rs` with the following scaffold code:

    ```rust
    --8<-- 'code/develop/rollups/customize-rollup/make-custom-pallets/barebone-pallet-template.rs'
    ```
...
```rust
--8<-- 'code/develop/rollups/customize-rollup/make-custom-pallets/pallet-config.rs'
```
...
```rust
--8<-- 'code/develop/rollups/customize-rollup/make-custom-pallets/pallet-event.rs'
```
...
```rust
--8<-- 'code/develop/rollups/customize-rollup/make-custom-pallets/pallet-error.rs'
```
...
```rust
--8<-- 'code/develop/rollups/customize-rollup/make-custom-pallets/pallet-storage.rs'
```
...
```rust
--8<-- 'code/develop/rollups/customize-rollup/make-custom-pallets/pallet-call.rs'
```
...
???code
    ```rust
    --8<-- 'code/develop/rollups/customize-rollup/make-custom-pallets/full-pallet.rs'
    ```
...
-   <span class="badge guide">Guide</span> __Testing__
    ...
    [:octicons-arrow-right-24: Reference](/develop/rollups/testing/)
...
-   <span class="badge guide">Guide</span> __Add a Pallet to the Runtime__
    ...
    [:octicons-arrow-right-24: Reference](/develop/rollups/customize-rollup/add-existing-pallets/)
