...
This guide explains how Polkadot's runtime versioning, Wasm deployment, and storage migrations enable these upgrades, ensuring the blockchain evolves smoothly and securely. You'll also learn how different upgrade processes apply to solo chains and rollups, depending on the network setup.
...
For rollups, upgrades are more complex. Rollups must first call `authorize_upgrade`, followed by `apply_authorized_upgrade`, to ensure the relay chain approves and applies the changes. Additionally, changes to current functionality that impact storage often require a [storage migration](#storage-migrations).
...
```rust
--8<-- 'code/develop/rollups/maintenance/runtime-upgrades/runtime_version.rs'
```
...
For further details about this process, see the [Storage Migrations](/develop/rollups/maintenance/storage-migrations/){target=\_blank} page.
