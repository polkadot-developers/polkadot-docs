...
```rust hl_lines="2"
--8<-- 'code/develop/rollups/testing/benchmarking/dispatchable-pallet-weight.rs'
```
...
--8<-- 'code/develop/rollups/testing/benchmarking/cargo.toml::1'
```
...
```toml title="Cargo.toml"
--8<-- 'code/develop/rollups/testing/benchmarking/cargo.toml:2:7'
```

Lastly, ensure that `frame-benchmarking` is included in `std = []`: 

```toml title="Cargo.toml"
--8<-- 'code/develop/rollups/testing/benchmarking/cargo.toml:8:12'
```
...
1. Navigate to your `runtime/src` directory and check if a `benchmarks.rs` file exists. If not, create one. This file will contain the macro that registers all pallets for benchmarking along with their respective configurations:
    ...
    --8<-- 'code/develop/rollups/testing/benchmarking/frame-benchmark-macro.rs'
    ```
    ...
    --8<-- 'code/develop/rollups/testing/benchmarking/frame-benchmark-macro.rs::3'
    );
    ```
...
3. Enable runtime benchmarking for your pallet in `runtime/Cargo.toml`:
    ```toml
    --8<-- 'code/develop/rollups/testing/benchmarking/runtime-cargo.toml'
    ```
...
--8<-- 'code/develop/rollups/testing/benchmarking/benchmark-output.html'
...
```rust title="lib.rs"
--8<-- 'code/develop/rollups/testing/benchmarking/weight-config.rs'
```
...
```rust title="mod.rs"
--8<-- 'code/develop/rollups/testing/benchmarking/runtime-pallet-config.rs'
```
