...
- **`Well known functions`** - already mapped test function
    ...
    === "Examples"
        ...
        alice: is up
        alice: rollup 100 is registered within 225 seconds
        alice: rollup 100 block height is at least 10 within 250 seconds
...
--8<-- 'code/develop/toolkit/rollups/spawn-chains/zombienet/write-tests/spawn-a-basic-chain-test.toml'
```

This test scenario checks to verify the following:

- Nodes are running
- The rollup with ID 100 is registered within a certain timeframe (255 seconds in this example)
- Rollup block height is at least a certain number within a timeframe (in this case, 10 within 255 seconds)
...
--8<-- 'code/develop/toolkit/rollups/spawn-chains/zombienet/write-tests/small-network-test.toml'
```

And the second test file:

```toml title="big-network-test.zndsl"
--8<-- 'code/develop/toolkit/rollups/spawn-chains/zombienet/write-tests/big-network-test.toml'
```
