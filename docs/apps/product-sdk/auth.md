...
- **`connect()` returns one of two shapes**: `{ kind: 'existing' }` when a session is already on disk, or `{ kind: 'qr', qrCode, login }` when the user has to scan. Print the QR *before* mounting any interface, then await `waitForLogin(handle, onStatus)`.
