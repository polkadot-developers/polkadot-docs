...
The code here targets `@parity/product-sdk` v0.30.0. It was written against `@parity/product-sdk-statement-store` v0.6.7 and `@parity/product-sdk-cloud-storage` v0.11.1, and every symbol it imports from them is still exported in v0.6.12 and v0.12.2. It needs at least `product-sdk` v0.30.0, the release that takes the Product's identity from the Host, and `statement-store` v0.5.0, which introduced the sponsored host-signing path.
...
!!! note "If Connect reports no accounts"
    `connectIdentity` throws `No accounts available from the host` when `connect()` succeeds but hands back an empty list. That is a real outcome, not a bug: the Host derives the account from the product ID it loaded the app under, and if it declines to derive one (for example, because you are signed out of Polkadot Desktop), you get zero accounts and no error. See [A Minimal Product](/apps/product-sdk/#a-minimal-product) for the same trap on the `createApp` path.
