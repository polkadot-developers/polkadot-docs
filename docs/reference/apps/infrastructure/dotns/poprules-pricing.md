...
The amount comes from a separate cost model, registered in the protocol registry and replaceable by governance. The deployed model charges one deposit for every name it admits, whatever its length, and that deposit is refundable rather than a purchase price. A length-sensitive model exists in the repository but is not the registered one, so a future release may price bands differently.
...
A name's band comes from its base length. An ordinary name is measured as written, digits included, so `web3` is four characters and `hamilton01` is 10.

| Base length  | Who may register                      | Deposit                          |
|:-------------|:--------------------------------------|:---------------------------------|
...
- **Names of five characters or fewer never reach the public path**: They enter circulation only through the reserved path, which mints an available label at no cost and skips the personhood check. That path requires either a governance-issued grant naming the label and the intended owner, or a Substrate Root origin.
- **Nothing shorter than three characters can be registered at all**: The public path rejects a shorter label outright. The gateway applies no length floor of its own, but a stem of five or fewer classifies as reserved, so it refuses one too.
...
Two durations bound the mechanism. `PopRules` holds a reservation slot for at most 12 weeks, and the controller's queue applies its own duration, which governance can configure. Once either lapses, the stem returns to whichever path its own length admits.
