...
| `DotnsCostModelRegistry`   | The registered cost model, which sets the deposit.                                   | `0x8bfd1f0957e73716732e725802f13830B5682da4` |
...
These addresses come from the `paseo-assethub` deployment artifact published with each release, on chain ID `420420417`. That artifact, and the `deployments.json` asset attached to the release, are the authoritative source if this table falls behind a tag.
...
Addresses are derived from a CREATE3 factory and a fixed salt namespace, so a redeployment of the same networks reproduces them. An unchanged address is not evidence of an unchanged interface: regenerate the ABIs you build against on every release.
