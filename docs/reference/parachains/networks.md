...
- **System-chain focused**: exposes the Paseo TestNet system parachains developers most commonly integrate against
- **Product-oriented**: designed for building and testing end-to-end product features that span multiple chains

Because the Devnet runs on Paseo TestNet system chains, it shares Paseo's native token, PAS. TestNet tokens are available from the [Polkadot faucet](https://faucet.polkadot.io/){target=\_blank}.

!!!note
    The Polkadot Products Devnet targets the community-run Paseo TestNet system chains and is distinct from other Paseo-based environments. It is intended for product and dApp development against Polkadot's system parachains, not for low-level protocol testing.
