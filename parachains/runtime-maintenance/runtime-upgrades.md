...
- Complete the [Install Polkadot SDK](/parachains/install-polkadot-sdk/){target=\_blank} guide.
- Set up and run the parachain template by following the [Set Up the Parachain Template](/parachains/launch-a-parachain/set-up-the-parachain-template/){target=\_blank} tutorial.
- Create and integrate a custom pallet by following the [Create a Custom Pallet](/parachains/customize-runtime/pallet-development/create-a-pallet/){target=\_blank} tutorial.
- Ensure access to [Polkadot.js Apps](https://polkadot.js.org/apps/#/explorer){target=\_blank} connected to your local node.
