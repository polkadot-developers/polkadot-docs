...
- **Sign In** establishes session identity. It runs once when the user lands in a Host (and re-runs if the session expires). It tells the Host who the user is so the Host can construct per-Product sub-accounts and surface Product UI.
- **`signAndSubmit`** signs a *specific transaction* with a Product's account. Every transaction requires its own approval in the App; there is no "auto-sign for the rest of this session" mode. This is intentional — see the [Accounts and Signing](/apps/build/accounts-and-signing/){target=\_blank} reference for the rationale.

A Product that wants to gate a feature on "is this the real user we paired with?" can rely on the session being authenticated already and instead reach for alias-based or `under_alias` patterns. Re-running Sign In is not the right primitive for that.
