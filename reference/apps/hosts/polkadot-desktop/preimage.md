...
A **preimage** is the original content of a hash that has been referenced on-chain — for example, the bytes of a governance proposal whose hash a referendum points at. The on-chain record holds only the hash; the preimage itself lives off-chain and must be made available for the referenced operation to execute. Polkadot Desktop exposes a preimage submission surface that Products can use through the Host API.

This page is the reference for that surface. The full Bulletin Chain reference (which preimage submission often interacts with) will land in the forthcoming **Infrastructure → Bulletin Chain** section.

## What the Preimage Surface Does

!!! warning "Provisional"
    The exact set of host calls Desktop exposes for preimage submission, the supported size limits, and the lifecycle hooks for preimage availability are still being finalized. This page documents the conceptual model; the API-level reference will be added once the surface is confirmed.

Conceptually, the preimage surface lets a Product:

- **Submit a preimage** for an on-chain operation that will reference it by hash. The Host signs the submission with the user's account (subject to the usual `ChainSubmit` permission and per-transaction approval on the paired App).
- **Provide hosting** for the preimage so it is reachable when the on-chain operation executes. In practice this often means writing the bytes to the Bulletin Chain and recording the resulting CID, but the exact storage path is part of the surface still being finalized.

## Statement vs Preimage

Both **statements** and **preimages** are off-chain content addressed by hash, and both surfaces look superficially similar — your Product submits some bytes, gets a hash back, and the on-chain world refers to that hash later. The two are *not* interchangeable, though, and choosing the wrong one is a real failure mode.

| Property | Statement (Statement Store) | Preimage |
|---|---|---|
| **What it's for** | Real-time signaling between users — chat, presence, typing indicators, multiplayer state | Off-chain content that an on-chain operation must dereference — governance proposals, scheduled call payloads |
