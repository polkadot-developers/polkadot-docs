...
description: How Polkadot Web resolves a .dot name and loads the resulting Product inside a browser tab — the visiting flow that distinguishes Web from Desktop.
...
The defining flow of Polkadot Web is *visiting*: a user navigates to a Polkadot Product by its `.dot` name from inside a regular browser tab. The mechanism is conceptually the same as Polkadot Desktop's address-bar resolution, but the delivery is different — there is no installed application, and the entry point is just a URL.

This page is the reference for that flow: what happens between the user typing a `.dot` name and the Product running inside the Web Host.
...
2. Web resolves the `.dot` name through DotNS, retrieving the content reference (CID) for the currently-published Product bundle.
...
- **No installer.** Desktop is a specialized browser the user installs; Web is just a URL.
...
- **Updates on load.** Each visit fetches the currently-published bundle. Web has no "update available" prompt — the user always gets whatever version DotNS resolves to right now.
