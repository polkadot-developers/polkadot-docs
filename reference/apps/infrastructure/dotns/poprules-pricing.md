...
!!! note "Self-declared PoP tier"
    dotNS currently reads PoP tier from a status the user sets themselves. On-chain verification against the People Chain is a forthcoming integration; until that ships, treat the tier check as cooperative, not adversarial.
...
- <span class="badge learn">Learn</span> **Proof of Personhood**
    ...
    Where PopRules reads the PoP tier from — the App-side mechanism that produces Full and Lite registrations.
