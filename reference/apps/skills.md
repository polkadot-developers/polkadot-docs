...
description: Reference for the Claude Code product-skills repo — modular skills that teach the Claude Code agent how to work effectively across the Polkadot Products stack.
...
| **[`bulletin-sdk`](https://github.com/paritytech/product-skills/tree/main/bulletin-sdk){target=\_blank}** | `@parity/bulletin-sdk` (JS) and `bulletin-sdk-rust` — the preferred SDK for Bulletin Chain storage in Products. Lower-level than `bulletin-storage`; reach for it when the agent needs the API-level details. |
...
**Step 1.** Register the repo as a skill marketplace:

```text
/plugin marketplace add paritytech/product-skills
```

**Step 2.** Install all skills at once:

```text
/plugin install polkadot-skills@product-skills
```

To pick individual skills instead, run `/plugin marketplace add paritytech/product-skills`, select **Browse and install plugins** → **product-skills**, choose the skills you want, and select **Install now**.
...
- <span class="badge external">External</span> **product-skills repo**
    ...
    The source of truth for the skills documented above — install commands, per-skill README files, and updates.
