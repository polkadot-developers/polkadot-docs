...
```js
--8<-- 'code/reference/tools/polkadot-js-api/api-instance.js'
```
...
- **Constants**: Values that are fixed in the runtime and don't change without a runtime upgrade.
    ...
    ```js
    --8<-- 'code/reference/tools/polkadot-js-api/read-constants.js'
    ```

- **State**: Current chain state that updates with each block.

    ```js
    --8<-- 'code/reference/tools/polkadot-js-api/read-state.js'
    ```
...
```js
--8<-- 'code/reference/tools/polkadot-js-api/send-txs.js'
```
