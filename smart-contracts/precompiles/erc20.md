...
The ERC20 precompile provides a standard ERC20 token interface for interacting with assets managed by the [Assets pallet](https://paritytech.github.io/polkadot-sdk/master/pallet_assets/index.html){target=\_blank}, helping smart contracts to interact with native Polkadot assets (such as USDT, USDC, and other tokens) using familiar Ethereum-style ERC20 calls. The precompile supports both Trust-Backed Assets (created directly on Polkadot Hub) and Foreign Assets (assets from other chains, identified by XCM Locations and exposed via the `0x0220` address prefix).
...
The ERC20 precompile distinguishes between two categories of assets, each with its own address prefix.
...
!!! warning
    The transfer will fail if the caller does not have sufficient balance.
