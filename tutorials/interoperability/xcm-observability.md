...
In complex XCM flows, such as transfers from one chain to another, you may want to use `SetTopic` to **consistently trace the message across all involved chains**.
...
During execution, the runtime includes a `SetTopic` instruction with the customised topic, which is preserved across chains:
