...
In previous tutorials, you learned how to [create a custom pallet](/tutorials/polkadot-sdk/rollups/zero-to-hero/build-custom-pallet/){target=\_blank} and [test it](/tutorials/polkadot-sdk/rollups/zero-to-hero/pallet-unit-testing/){target=\_blank}. The next step is to include this pallet in your runtime, integrating it into the core logic of your blockchain.
...
1. Open the `runtime/Cargo.toml` file and locate the `[dependencies]` section. Add the pallets with the following lines:
    ...
    --8<-- 'code/tutorials/polkadot-sdk/rollups/zero-to-hero/runtime/Cargo.toml:19:19'
    ...
    --8<-- 'code/tutorials/polkadot-sdk/rollups/zero-to-hero/runtime/Cargo.toml:74:75'
    ```

2. In the `[features]` section, add the pallets to the `std` feature list:

    ```toml hl_lines="5-6" title="Cargo.toml"
    --8<-- 'code/tutorials/polkadot-sdk/rollups/zero-to-hero/runtime/Cargo.toml:77:79'
      ...
    --8<-- 'code/tutorials/polkadot-sdk/rollups/zero-to-hero/runtime/Cargo.toml:132:134'
    ```
...
1. Add the `OriginCaller` import:
    ...
    ```rust
    --8<-- 'code/tutorials/polkadot-sdk/rollups/zero-to-hero/runtime/src/configs/mod.rs:56:56'
    ```
...
2. Implement the [`Config`](https://paritytech.github.io/polkadot-sdk/master/pallet_utility/pallet/trait.Config.html){target=\_blank} trait for both pallets at the end of the `runtime/src/config/mod.rs` file:
    ...
    ```rust title="mod.rs"
    --8<-- 'code/tutorials/polkadot-sdk/rollups/zero-to-hero/runtime/src/configs/mod.rs:314:330'
    --8<-- 'code/tutorials/polkadot-sdk/rollups/zero-to-hero/runtime/src/configs/mod.rs:332:332'
    ```

3. Locate the `#[frame_support::runtime]` macro in the `runtime/src/lib.rs` file and add the pallets:

    ```rust hl_lines="5-9" title="lib.rs"
    --8<-- 'code/tutorials/polkadot-sdk/rollups/zero-to-hero/runtime/src/lib.rs:253:255'
        ...
    --8<-- 'code/tutorials/polkadot-sdk/rollups/zero-to-hero/runtime/src/lib.rs:315:320'
    ```
...
3. Verify you can interact with the new pallets using the [Polkadot.js Apps](https://polkadot.js.org/apps/?rpc=ws%3A%2F%2F127.0.0.1%3A9944#/extrinsics){target=\_blank} interface. Navigate to the **Extrinsics** tab and check that you can see both pallets:
    - Utility pallet
        ...
        ![](/images/tutorials/polkadot-sdk/rollups/zero-to-hero/add-pallets-to-runtime/add-pallets-to-runtime-1.webp)
    

    - Custom pallet

        ![](/images/tutorials/polkadot-sdk/rollups/zero-to-hero/add-pallets-to-runtime/add-pallets-to-runtime-2.webp)
...
-   <span class="badge tutorial">Tutorial</span> __Pallet Benchmarking (Optional)__
    ...
    [:octicons-arrow-right-24: Get Started](/tutorials/polkadot-sdk/rollups/zero-to-hero/pallet-benchmarking/)
