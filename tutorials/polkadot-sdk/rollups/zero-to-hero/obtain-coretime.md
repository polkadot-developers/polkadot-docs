...
After deploying a rollup to the Paseo TestNet in the [Deploy to TestNet](/tutorials/polkadot-sdk/rollups/zero-to-hero/deploy-to-testnet/){target=\_blank} tutorial, the focus shifts to understanding Coretime, which is the mechanism in which validation resources are allocated from the relay chain to the respective task, such as a rollup. A rollup could only produce blocks and finalize them on the relay chain by obtaining coretime.
...
- Assign a task / rollup to the core for block production
...
- A registered parathread with the correct genesis, runtime, and rollup ID that matches the chain specification
- A properly configured and synced (with the relay chain) collator

Once the above is complete, obtaining coretime is the last step to enable your rollup to start producing and finalizing blocks using the relay chain's validator set. If you don't, refer to the previous tutorial: [Deploy on Paseo TestNet](/tutorials/polkadot-sdk/rollups/zero-to-hero/deploy-to-testnet/){target=\_blank}.
...
To produce a block in your rollup, navigate to Polkadot.js Apps and ensure you're connected to the Paseo relay chain. Then, access the [**Developer > Extrinsics**](https://polkadot.js.org/apps/#/extrinsics){target=\_blank} tab and execute the `onDemand.placeOrderAllowDeath` extrinsic from the account that registered the `ParaID`. For this example, `maxAmount` is set to `1000000000000` (this value may vary depending on the network conditions), and `paraId` is set to `4518`:

![](/images/tutorials/polkadot-sdk/rollups/zero-to-hero/obtain-coretime/obtain-coretime-9.webp)

With each successful on-demand extrinsic, the rollup will produce a new block. You can verify this by checking the collator logs. If the extrinsic is successful, you should see output similar to the following:

--8<-- 'code/tutorials/polkadot-sdk/rollups/zero-to-hero/obtain-coretime/obtain-coretime-1.html'
...
![](/images/tutorials/polkadot-sdk/rollups/zero-to-hero/obtain-coretime/obtain-coretime-1.webp)
...
![](/images/tutorials/polkadot-sdk/rollups/zero-to-hero/obtain-coretime/obtain-coretime-2.webp)
...
![Screenshot of the RegionX app displaying the main interface.](/images/tutorials/polkadot-sdk/rollups/zero-to-hero/obtain-coretime/obtain-coretime-3.webp)

On the top left is a network switch. Ensure you have selected your rollup and that it is registered before purchasing a core.
...
![](/images/tutorials/polkadot-sdk/rollups/zero-to-hero/obtain-coretime/obtain-coretime-4.webp)
...
![](/images/tutorials/polkadot-sdk/rollups/zero-to-hero/obtain-coretime/obtain-coretime-5.webp)
...
![](/images/tutorials/polkadot-sdk/rollups/zero-to-hero/obtain-coretime/obtain-coretime-6.webp)

Congratulations, you just purchased a core using RegionX! You can assign the core to your rollup, partition, interlace, and more using RegionX.
...
![](/images/tutorials/polkadot-sdk/rollups/zero-to-hero/obtain-coretime/obtain-coretime-7.webp)

Click the **Add Task** button and input the rollup identifier, along with the name of your project, and finalize it by clicking **Add Task**.

![](/images/tutorials/polkadot-sdk/rollups/zero-to-hero/obtain-coretime/obtain-coretime-8.webp)
...
Once you sign and send this transaction, your rollup will be assigned to that core.